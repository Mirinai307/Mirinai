...
> **로드 정책**: 이 파일은 평소 작업에서 읽지 않는다. **시스템 파일(_shared/·_templates/·CLAUDE.md)을 수정·검증하는 작업일 때만** orchestrator가 읽는다. (progressive disclosure — `orchestrator-rules.md` §2 프로토콜 참조)
...
- knot vault: https://github.com/netwaif/knot
...
| Latent briefing (task-guided 압축) | brief = 그 worker의 그 작업용으로 재구성 | "앞 작업 요약 그대로 전달" 금지 |
...
| Fan-out/Fan-in + 작업 재진입 | routing.md 토폴로지표·Fan-in 규칙, orchestrator-rules §3 | 병렬 통합·재진입 분기는 기존 append-only+provenance·never-trust-upstream 재사용 |

## 2. 권위 우선순위 (Context Clash 해소 규칙)

`CLAUDE.md` > `_shared/routing.md`·`approval-policy.md`·`orchestrator-rules.md` > 외부 매뉴얼.
충돌 발견 시 낮은 쪽을 높은 쪽에 맞추고 log.md에 남긴다.

## 3. 이미 내린 결정 (재논의 금지, 뒤집으려면 근거 갱신)

- **D1** `write_scope` 값 집합 = `none | tasks-only | "패턴"`. `tasks-only`=codex-main 기본(tasks/<task>/ 내부만).
- **D2** codex-critic 선행조건 = "리뷰 대상 산출물 경로 존재 — 보통 claude-main result.md, 또는 brief에 명시된 기존 코드·문서·소스". claude-main 전용 아님.
- **D3** context.md 구조 = 4섹션 유지. `Files Modified/Decisions Made` 헤딩 도입 금지.
- **D4** gemini 백엔드 = **Antigravity `agy` CLI**(`_shared/backends.json` 정본, 디스패처 `_shared/adapters/call_worker.sh`). 기본 모델 = `gemini-3.1-pro-high`. 옛 `mcp__gemini-pro__*` 프록시 브리지·CLI 래퍼 `mcp__gemini__*`는 **폐기**.
- **D5** MultiAgent 작업은 인터랙티브 세션 전용, worktree/백그라운드 세션 금지.
- **D6** 작업 재진입 프로토콜 + 토폴로지 4패턴 채택, Supervisor·Hierarchical Delegation **배제**. 근거: 단일 orchestrator, worker간 무통신, file-as-memory와 충돌.
- **D7** 모델 식별자 표기 정책 = 휘발성 높은 식별자는 별칭으로. claude-main=별칭 `opus`, codex=config.toml 기본값, gemini=backends.json에 명시 핀.
- **D8** 카파시 4원칙 층별 적용 = 오케스트레이터 풀버전 / 워커층 유일 정본은 `_templates/worker-brief.md`의 "Worker 행동 규약" 고정 블록.
- **D9** knot 배포 = `knot/` 서브디렉토리에 vault 직접 포함. `$KNOT_VAULT=/home/user/Mirinai/knot` 으로 설정.
