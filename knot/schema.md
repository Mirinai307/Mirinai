...
- vault를 변경하는 실행(save, ingest, 자동수정 있는 lint, note를 저장한 query)은 **git commit으로 마무리**한다.
- 실행 시작 시 working tree가 더러우면 중단하고 보고한다.
- 커밋 메시지: `ingest: <제목>` / `lint: <요약>` / `query: <제목>`. 트레일러에 실행한 실제 모델명을 남긴다.
...
이 vault의 경로: `KNOT_VAULT=/home/user/Mirinai/knot`

```bash
export KNOT_VAULT=/home/user/Mirinai/knot
```
